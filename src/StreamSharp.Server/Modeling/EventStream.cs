...
[GenerateId]
public sealed class EventStream : IReadOnlyCollection<EventRecord>
{
    private EventRecord[] _events = [];
    private readonly List<EventRecord> _uncomitted = [];
    ...
    public static EventStream Create(EventRecord[]? events = null)
        => new(EventStreamId.New())
        {
            _events = events ?? [],
        };
    ...
    private EventStream(EventStreamId id)
    {
        Id = id;
    }
}
